...
- **Naming**: `snake_case` methods, class per item variant (`Hacker::News::Story < Hacker::News::Item`), gem `hacker-news-client`.
- **Errors**: `Hacker::News::Error < StandardError` with subclasses; raised, never returned.
...
- **Packaging**: `hacker-news-client.gemspec`, `lib/hacker/news/client.rb` (entry), `lib/hacker/news/version.rb`, `lib/hacker-news-client.rb` (Bundler shim), `Rakefile`, `required_ruby_version >= 3.1`.
...
- `scripts/bump-version.sh` propagates into each manifest: `js/package.json`, `ts/package.json`, `python/pyproject.toml`, `ruby/lib/hacker/news/version.rb`, `go/version.go`, `rust/Cargo.toml`.
