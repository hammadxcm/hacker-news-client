...
# Pure unit tests — mocks HackerNewsClient's transport; no network, no
...
class ConstructorTests < Minitest::Test
  def test_defaults
    ...
    assert_equal HackerNewsClient::Client::DEFAULT_TIMEOUT, c.timeout
    assert_equal HackerNewsClient::Client::DEFAULT_CONCURRENCY, c.concurrency
    assert c.user_agent.start_with?('hn-client-ruby/')
  end

  def test_strips_trailing_slash
    c = HackerNewsClient::Client.new(base_url: 'http://x/v0///')
...
class ItemDecodeTests < Minitest::Test
  ...
  def test_story
    assert_kind_of HackerNewsClient::Story, @client.item(1)
  end

  def test_every_variant
    assert_kind_of HackerNewsClient::Comment, @client.item(2)
    assert_kind_of HackerNewsClient::Job,     @client.item(3)
    assert_kind_of HackerNewsClient::Poll,    @client.item(4)
    assert_kind_of HackerNewsClient::PollOpt, @client.item(5)
  end
  ...
  def test_unknown_type_raises_argument_error
    assert_raises(ArgumentError) do
      HackerNewsClient::Item.from_hash('type' => 'future', 'id' => 1)
    end
  end
end
...
class ErrorMappingTests < Minitest::Test
  def test_http_error
    ...
    err = assert_raises(HackerNewsClient::HttpError) { c.item(1) }
  ...
  def test_json_error
    ...
    assert_raises(HackerNewsClient::JsonError) { c.item(1) }
  end
  ...
  def test_timeout_error
    c = HackerNewsClient::Client.new(
      base_url: BASE,
      transport: fake_transport(
        "#{BASE}/item/1.json" => HackerNewsClient::TimeoutError.new('hn: timeout', url: 'x')
      )
    )
    assert_raises(HackerNewsClient::TimeoutError) { c.item(1) }
  end
  ...
  def test_transport_error
    c = HackerNewsClient::Client.new(
      base_url: BASE,
      transport: fake_transport(
        "#{BASE}/item/1.json" => HackerNewsClient::TransportError.new('hn: down', url: 'x')
      )
    )
    assert_raises(HackerNewsClient::TransportError) { c.item(1) }
  end
  ...
  def test_default_transport_connection_refused
    ...
    c = HackerNewsClient::Client.new(base_url: 'http://127.0.0.1:1/v0', timeout: 0.5)
    assert_raises(HackerNewsClient::TransportError) { c.item(1) }
  end
end
...
class BatchAndHydrateTests < Minitest::Test
  ...
  def test_items_fail_fast
    ...
    assert_raises(HackerNewsClient::HttpError) { c.items([1, 99, 2, 3]) }
  end
...
class CommentTreeTests < Minitest::Test
  ...
  def test_error_during_tree_fetch_propagates
    ...
    assert_raises(HackerNewsClient::HttpError) { c.comment_tree(1) }
  end
  ...
  def test_semaphore_contention_concurrency_one
    ...
    c = HackerNewsClient::Client.new(base_url: BASE, concurrency: 1, transport: slow_transport)
    root = c.comment_tree(100)
  ...
  def test_error_during_tree_fetch_cancels_siblings
    ...
    c = HackerNewsClient::Client.new(base_url: BASE, concurrency: 4, transport: t)
    assert_raises(HackerNewsClient::HttpError) { c.comment_tree(100) }
  end
end
